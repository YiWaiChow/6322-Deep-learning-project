...
class Segmentic_Pvt(nn.Module):

    def __init__(self, num_blocks, num_classes, channels, height, width, batch_size):
        super(Segmentic_Pvt, self).__init__()
        self.in_planes = 64
        ...
        self.back_bone = segmentation_pvt(
        ...
        self.toplayer = nn.Conv2d(
            512, 256, kernel_size=1, stride=1, padding=0)  # Reduce channels
...
if __name__ == "__main__":
    model = Segmentic_Pvt([2, 4, 23, 3], 150, channels=3,
                          height=512, width=512, batch_size=2)
    input = torch.rand(2, 3, 512, 512)
    output = model(input)
    print(output.size())

    image = cv2.imread("./ADE_train_00000001.png")
